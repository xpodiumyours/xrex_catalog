...
class XRexTfliteObjectDetectionService implements XRexObjectDetectionServiceInterface {
  ...
  Future<List<XRexDetectedRegion>> detectObjects({
    ...
    final regions = await _detectObjectsFromImageBytes(imageBytes);
    return regions;
  }

  Future<List<XRexDetectedRegion>> _detectObjectsFromImageBytes(
    Uint8List imageBytes,
  ) async {
    final decoded = img.decodeImage(imageBytes);
    if (decoded == null) return const [];

    // Use decoded image dimensions for the interface call
    ...
    try {
      ...
      if (useQuantizedModel && e.toString().contains('asset')) {
        ...
        ).detectObjects(imageBytes: imageBytes, width: imageWidth, height: imageHeight);
      }
      rethrow;
    } finally {
      interpreter?.close();
  ...
  List<XRexDetectedRegion> _parseOutputs({
    ...
    for (var i = 0; i < detectedCount; i++) {
      ...
      final classId = i < classRows.length ? _toDouble(classRows[i]).round() : null;
...
class XRexMlkitObjectDetectionService implements XRexObjectDetectionServiceInterface {
  const XRexMlkitObjectDetectionService({this.scoreThreshold = 0.40, this.maxResults = 12});
  ...
  Future<List<XRexDetectedRegion>> detectObjects({
    ...
    try {
      final inputImage = InputImage.fromBytes(bytes: imageBytes, metadata: InputImageMetadata(
      ...
      for (var i = 0; i < detectedObjects.length; i++) {
        ...
        final label = obj.labels.isNotEmpty ? obj.labels.first.text : 'MLKit ürün';
        
        regions.add(
          XRexDetectedRegion(
            id: 'mlkit_region_${i + 1}',
      ...
      regions.sort((a, b) => (b.confidence ?? 0).compareTo(a.confidence ?? 0));
      return regions.take(maxResults).toList();
    } catch (e) {
      return const [];
    }
  }
